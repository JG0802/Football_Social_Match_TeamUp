...
// 사용자 초대코드 목록 조회 API
app.post('/invite/user-codes', (req, res) => {
    ...
    try {
        ...
        const userCodes = inviteCodes
            .filter(code => code.users.includes(username))
            .map(code => code.inviteCode);

        return res.status(200).json({ success: true, inviteCodes: userCodes });
    } catch (error) {
        console.error('초대코드 탐색 중 오류 발생:', error);
        return res.status(500).json({ success: false, message: '서버 내부 오류가 발생했습니다.' });
    }
});
...
app.post('/invite/save', (req, res) => {
    const { positions, inviteCode } = req.body;

    if (!positions || !Array.isArray(positions) || !inviteCode) {
        return res.status(400).json({ success: false, message: '입력 데이터가 유효하지 않습니다.' });
    }
...
// 사용자 선호 포지션을 반환하는 API
app.post('/invite/usersAndPositions', (req, res) => {
    const { inviteCode } = req.body;
    ...
    try {
        ...
        inviteCodeEntry.users.forEach(username => {
            const user = users.find(u => u.username === username);
            if (user) {
                usersWithPositions[username] = user.positions; // 사용자 선호 포지션 추가
            }
        });

        return res.status(200).json({
            success: true,
            users: inviteCodeEntry.users,
            positions: usersWithPositions
        });
    } catch (error) {
        console.error('사용자 및 포지션 데이터를 가져오는 중 오류 발생:', error);
        return res.status(500).json({ success: false, message: '서버 내부 오류가 발생했습니다.' });
    }
});
